...
impl<E> VecWrapper<E> {
    ...
    pub(super) fn is_double(&self) -> bool {
        match self {
    ...
    pub(super) fn is_valid(&self) -> ReRes<()> {
        match self {
    ...
    pub(super) fn att(&self, r: usize, c: usize) -> &E {
        match self {
    ...
    pub(super) fn att_mut(&mut self, r: usize, c: usize) -> &mut E {
        match self {
...
impl<E: Clone> VecWrapper<E> {
    ...
    pub(super) fn resize(mut self, r: usize, c: usize, with: E) -> Self {
        self = match self {
...
impl<E: Default + Clone> VecWrapper<E> {
    ...
    pub(super) fn resize_default(mut self, r: usize, c: usize) -> Self {
        self = match self {
            Self::Single(mut single) => {
...
impl<E: Clone> RawGrid<E> {
    ...
    pub fn append_rows(mut self, tail: Self, t: bool) -> ReRes<Self> {
        if self.cols(false) != tail.cols(t) {
            return Err(GridErr(RowsAppendMismatch {
                dest: self.cols(false),
                tail: tail.cols(t),
            }));
        }
        ...
        self.vec = match self.trans {
            false => self
                .vec
                .resize(ex_rows + new_rows, cols, tail.att(0, 0, false).clone()),
            true => self
                .vec
                .resize(ex_rows, cols + new_rows, tail.att(0, 0, false).clone()),
        };
        for r in 0..new_rows {
            let er = ex_rows + r;
            for c in 0..cols {
    ...
    pub fn append_cols(mut self, tail: Self, t: bool) -> ReRes<Self> {
        if self.rows(false) != tail.rows(t) {
            return Err(GridErr(ColsAppendMismatch {
                dest: self.rows(false),
                tail: tail.rows(t),
            }));
        }
        ...
        self.vec = match self.trans {
            false => self
                .vec
                .resize(rows, ex_cols + new_cols, tail.att(0, 0, false).clone()),
            true => self
                .vec
