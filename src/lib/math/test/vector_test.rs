use {
    super::super::{matrix::Matrix, set_biform_identity, set_biform_vec},
    crate::{
...
fn vector_combination() {
    ...
    let comb = Matrix::from_single(vec![-50.0, -22.0, -23.0])
        .raw_transpose()
        .to_col();
...
fn vector_multicol_dim() {
    let lhs = Matrix::from_double(vec![vec![1.0, 2.0, 3.0], vec![3.0, 4.0, 5.0]]).to_multicol();
    assert_eq!(lhs.dim().unwrap(), 2);
}

#[test]
fn vector_multirow_dim() {
    let lhs = Matrix::from_double(vec![vec![1.0, 2.0, 3.0], vec![3.0, 4.0, 5.0]]).to_multirow();
...
fn scalar_prod() {
    ...
    let rhs = Matrix::from_double(vec![vec![3.0, 4.0, 5.0]]).to_row();
