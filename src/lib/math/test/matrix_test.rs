use {
    super::super::{matrix::Matrix, set_biform_identity, set_precision},
    crate::{
...
fn determinant_of_non_square() {
    let m = Matrix::from_double(vec![vec![2.0, 0.0, -5.0], vec![0.0, -3.0, 0.0]]);
    assert_eq!(m.det(), Err(GridErr(NotSquare((2, 3)))));
}

#[test]
fn determinant_of_non_square_trans() {
    let m = Matrix::from_double(vec![vec![2.0, 0.0, -5.0], vec![0.0, -3.0, 0.0]]).transpose();
...
#[test]
fn round_matr() {
    let m = Matrix::from_double(vec![vec![1.5625, 0.875], vec![-0.875, -0.75]]);
    let m_round = Matrix::from_double(vec![vec![1.5, 0.5], vec![-0.5, -0.5]]);
    assert_eq!(m.round_prec(1), m_round);
}

#[test]
fn add_assign_matr() {
    let lhs = Matrix::from_double(vec![vec![1.0, 2.0], vec![3.0, 4.0]]);
    let rhs = Matrix::from_double(vec![vec![5.0, 6.0], vec![8.0, 7.0]]);
    let sum = Matrix::from_double(vec![vec![6.0, 8.0], vec![11.0, 11.0]]);
...
fn add_t_matr() {
    let lhs = Matrix::from_double(vec![vec![1.0, 2.0], vec![3.0, 4.0]]);
    let rhs = Matrix::from_double(vec![vec![5.0, 6.0], vec![8.0, 7.0]]);
    let sum = Matrix::from_double(vec![vec![6.0, 10.0], vec![9.0, 11.0]]);
    assert_eq!(lhs.add_t(&rhs), sum);
}

#[test]
fn add_assign_trans_matr() {
    let lhs = Matrix::from_double(vec![vec![1.0, 2.0, 10.0], vec![3.0, 4.0, 11.0]]);
    let rhs = Matrix::from_double(vec![vec![5.0, 6.0], vec![7.0, 8.0], vec![0.0, 9.0]]).transpose();
    let sum = Matrix::from_double(vec![vec![6.0, 9.0, 10.0], vec![9.0, 12.0, 20.0]]);
    assert_eq!(lhs.add_assign(&rhs), sum);
}
...
fn add_assign_t_trans_matr() {
    let lhs = Matrix::from_double(vec![vec![1.0, 2.0, 10.0], vec![3.0, 4.0, 11.0]]);
    let rhs = Matrix::from_double(vec![vec![5.0, 6.0, 100.0], vec![7.0, 8.0, 200.0]]).transpose();
    let sum = Matrix::from_double(vec![vec![6.0, 8.0, 110.0], vec![10.0, 12.0, 211.0]]);
    assert_eq!(lhs.add_assign_t(&rhs), sum);
}

#[test]
fn sub_assign_diff_matr() {
    let lhs = Matrix::from_double(vec![vec![1.0, 2.0], vec![3.0, 4.0]]).to_multicol();
    let rhs = Matrix::from_double(vec![vec![5.0, 6.0], vec![8.0, 7.0]]).to_multirow();
    let sum = Matrix::from_double(vec![vec![6.0, 8.0], vec![11.0, 11.0]]).to_multicol();
...
fn invalid_add_matr() {
    let lhs = Matrix::from_double(vec![vec![1.0, 2.0], vec![3.0, 4.0]]);
    let rhs = Matrix::from_double(vec![vec![5.0, 6.0], vec![8.0, 7.0], vec![8.0, 7.0]]);
    assert_eq!(
        lhs.add(&rhs),
        Matrix::Failure(MathErr(AddSizesMismatch {
            lhs: (2, 2),
            rhs: (3, 2)
        }))
    );
...
fn invalid_add_t_matr() {
    let lhs = Matrix::from_double(vec![vec![1.0, 2.0], vec![3.0, 4.0]]);
    let rhs = Matrix::from_double(vec![vec![5.0, 6.0], vec![8.0, 7.0], vec![8.0, 7.0]]).transpose();
    assert_eq!(
        lhs.add_t(&rhs),
        Matrix::Failure(MathErr(AddSizesMismatch {
            lhs: (2, 2),
...
#[test]
fn mul_matr() {
    let lhs = Matrix::from_double(vec![vec![4.0, 1.0, 0.0], vec![-1.0, 0.0, 1.0]]);
    let rhs = Matrix::from_double(vec![vec![1.0, 1.0, -3.0]]).transpose();
...
fn mul_t_matr() {
    ...
    let rhs = Matrix::from_double(vec![vec![1.0, 1.0, 0.0], vec![-2.0, 4.0, 3.0]]);
    let prod = Matrix::from_double(vec![vec![5.0, -4.0], vec![-1.0, 5.0], vec![1.0, 37.0]]);
...
fn invalid_mul_t_matr() {
    ...
    let rhs = Matrix::from_double(vec![vec![1.0, 1.0], vec![-2.0, 4.0]]);
    assert_eq!(
        lhs.mul_t(&rhs),
        Matrix::Failure(MathErr(MulSizesMismatch {
            lhs: (3, 3),
            rhs: (2, 2)
        }))
    );
...
#[test]
fn col_norm() {
    let c = Matrix::from_single(vec![-1.0, -3.0, -7.0])
        .raw_transpose()
        .to_col();
...
#[test]
fn neg_rowlist() {
    let rl = Matrix::from_double(vec![vec![0.0, 1.0, 2.0], vec![-1.0, -2.0, -3.0]]);
    let neg = Matrix::from_double(vec![vec![0.0, -1.0, -2.0], vec![1.0, 2.0, 3.0]]);
    assert_eq!(rl.neg(), neg);
}

#[test]
fn neg_col() {
    let rl = Matrix::from_single(vec![0.0, 1.0, 2.0])
        .raw_transpose()
        .to_col();
    let neg = Matrix::from_double(vec![vec![0.0], vec![-1.0], vec![-2.0]]).to_col();
    assert_eq!(rl.neg(), neg);
}
