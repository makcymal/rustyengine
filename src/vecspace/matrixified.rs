...
pub trait Matrixified {
    ...
    fn m_add(&self, rhs: &impl Matrixified<Elem=Self::Elem>, rhs_sign: Sign)
    ...
    fn m_mul_by_num(&mut self, num: Self::Elem)
    where Self::Elem: Num
    {
...
impl<T: Num> Div<&Matrix<T>> for &Vector<T> {
    type Output = Result<Matrix<T>, MatrixifiedError>;
